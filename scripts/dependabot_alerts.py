...
def permission_hint(headers: dict[str, str], payload: Any) -> str:
    ...
    lines.extend(
        [
            "",
            "Fix:",
            "  1. Fine-grained PAT → Repository permissions →",
            "     Dependabot alerts = Read-only",
            "     (API name: vulnerability_alerts).",
            "     Not Dependabot secrets / Code scanning alerts.",
            "  2. Store that PAT as Cloud Agent secret GH_TOKEN",
            "     (replace any previous value; gh ignores GITHUB_PAT).",
            "  3. Or use a classic PAT with security_events / repo scope.",
            "  4. Re-run: python3 scripts/dependabot_alerts.py --check",
            "",
